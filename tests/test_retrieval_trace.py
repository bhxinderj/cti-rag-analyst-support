...
def test_hybrid_mode_preserves_exact_entity_hit_outside_rrf_top_k():
    retriever = object.__new__(HybridRetriever)
    retriever.mode = "hybrid"
    ...
    exact_hit = RetrievedChunk(
        doc_id="nvd_CVE-2024-3094",
        content="Title: CVE-2024-3094\nContent: XZ Utils backdoor.",
        score=1.0,
        metadata={"source": "nvd", "title": "CVE-2024-3094"},
        rank=14,
    )
    bm25_hits = [
        RetrievedChunk(
            doc_id=f"bm25_other_{idx}",
            content=f"Title: Other {idx}\nContent: Other.",
            score=20 - idx,
            metadata={"source": "nvd", "title": f"Other {idx}"},
            rank=idx,
        )
        for idx in range(14)
    ] + [exact_hit]
    vector_hits = [
        RetrievedChunk(
            doc_id=f"vector_{idx}",
            content=f"Title: Vector {idx}\nContent: Vector match.",
            score=0.9 - idx * 0.01,
            metadata={"source": "misp", "title": f"Vector {idx}"},
            rank=idx,
        )
        for idx in range(5)
    ]
    fused_hits = bm25_hits[:10]

    calls = []

    retriever._search_bm25 = lambda query, top_k: bm25_hits
    retriever._search_vector = lambda query, top_k: vector_hits
    retriever._reciprocal_rank_fusion = lambda bm25_results, vector_results: fused_hits

    def fake_rerank(query, chunks):
        calls.append([chunk.doc_id for chunk in chunks])
        return chunks[:5]

    retriever._rerank = fake_rerank

    HybridRetriever.retrieve(retriever, "What is CVE-2024-3094 and what is its impact?")

    assert "nvd_CVE-2024-3094" in calls[0]
    assert retriever.last_trace["stages"]["fused"][-1]["doc_id"] == "nvd_CVE-2024-3094"


def test_preserve_query_entity_hits_keeps_rrf_order_and_deduplicates():
    retriever = object.__new__(HybridRetriever)
    query_entities = ["cve-2024-3094"]

    fused = [
        RetrievedChunk(
            doc_id="kept_doc",
            content="Title: Kept\nContent: Already fused.",
            score=0.5,
            metadata={"title": "Kept"},
            rank=0,
        ),
        RetrievedChunk(
            doc_id="nvd_CVE-2024-3094",
            content="Title: CVE-2024-3094\nContent: Exact hit already present.",
            score=0.4,
            metadata={"title": "CVE-2024-3094"},
            rank=1,
        ),
    ]
    bm25_results = [
    ...
    preserved = HybridRetriever._preserve_query_entity_hits(
        retriever,
        fused,
        query_entities,
        bm25_results,
    ...
    assert [chunk.doc_id for chunk in preserved] == [
        "kept_doc",
        "nvd_CVE-2024-3094",
        "cisa_kev_CVE-2024-3094",
    ]
